...
def train_using_conditional_gen_eval(model, train_dataloader, eval_dataloader, tokenizer, optimizer, lr_scheduler,
                                     gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None,
                                     rank=None, wandb_run=None):
    """
    Trains the model on the given dataloader
    ...
    for epoch in range(train_config.num_epochs):
        ...
        lr_scheduler.step()
        if train_config.run_validation:
            scores = evaluation_conditional_gen(model, train_config, eval_dataloader, local_rank, tokenizer, wandb_run, epoch)
            if train_config.save_metrics:
                scores_list.append("epoch " + str(epoch) + ":" + str(scores))
            eval_epoch_metric = 0.4 * scores["BLEU_4"] + 0.3 * scores["BLEU_1"] + 0.3 * scores["CIDEr"]
            ...
            if train_config.save_model and eval_epoch_metric > best_val_metric:
                ...
                if train_config.use_peft:
                    if train_config.enable_fsdp:
                        ...
                        print(f"we are about to save the PEFT modules")
                    model.save_pretrained(train_config.output_dir)  # 保存权重
                    if train_config.enable_fsdp:
                        if rank == 0:
                            print(f"PEFT modules are saved in {train_config.output_dir} directory")
                    else:
...
def evaluation_conditional_gen(model, train_config, eval_dataloader, local_rank, tokenizer, wandb_run, epoch):
    model.eval()
    sample_ids = []
    eval_preds = []
    ground_truth = []
    ...
    save_file = f"{train_config.output_dir}/Eval_result_Epoch{epoch}.json"
